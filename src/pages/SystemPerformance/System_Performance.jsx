  ...
  useEffect(() => {
    ...
    const unsub = window.api.onWsMessage((msg) => {
      if (msg?.type === "metrics:performance:result" && msg?.requestId === reqIdRef.current) {
        ...
        if (msg.error) {
          notify?.error?.(
            msg.error === "forbidden" ? tr("performance.permissionDenied", "Permission denied") : msg.error,
            tr("performance.title", "System Performance")
          );
          setMetrics(null);
        } else {
          setMetrics({
      ...
      <div className="devicesPage devicesPage--full">
        <header className="devicesHeader">
          ...
          <div className="devicesHeaderText">
            <h1 className="devicesTitle">{tr("performance.title", "System Performance")}</h1>
            <p className="devicesSubtitle">{tr("performance.noPermission", "You don't have permission to view system metrics.")}</p>
          </div>
        </header>
      </div>
    );
      ...
      <header className="devicesHeader">
        ...
        <div className="devicesHeaderText">
          ...
          <p className="devicesSubtitle">
            {tr("performance.subtitle", "Memory usage, API response times, and MongoDB database stats. Refreshes on demand.")}
          </p>
        </div>
      </header>

      <main className="devicesMain">
        <Tippy content={tr("performance.refreshMetrics", "Refresh metrics")} animation="shift-away" placement="bottom" delay={[200, 0]}>
          <button
            type="button"
          ...
          <span>{loading ? tr("common.loading", "Loading…") : tr("performance.refresh", "Refresh")}</span>
        </button>
        </Tippy>
        ...
        {loading && !metrics ? (
          <div className="devicesLoading">
            ...
            <p>{tr("performance.loadingMetrics", "Loading metrics…")}</p>
          </div>
        ) : !loading && !metrics ? (
          <div className="devicesEmpty">
            <img src={getAssetUrl("assets/svg/nodata-ill.svg")} alt="" className="devicesEmptyIllustration" />
            <p>{tr("performance.empty", "No metrics available or permission denied.")}</p>
          </div>
        ) : metrics ? (
          <>
            <section className="perfSection perfSection--animate">
              <h2 className="perfSectionTitle">
                ...
                {tr("performance.memorySection", "Memory usage (Node.js process)")}
              </h2>
              <div className="perfGrid">
                <div className="perfCard" style={{ animationDelay: "0s" }}>
                  ...
                  <p className="perfCardSub">{tr("performance.rssSub", "Resident set size")}</p>
                </div>
                <div className="perfCard" style={{ animationDelay: "0.05s" }}>
                  <p className="perfCardTitle">{tr("performance.heapUsed", "Heap used")}</p>
                  <p className="perfCardValue">
                    {metrics.memory?.heapUsedMb != null ? `${metrics.memory.heapUsedMb} MB` : "—"}
                  </p>
                  <p className="perfCardSub">{tr("performance.heapUsedSub", "V8 heap used")}</p>
                </div>
                <div className="perfCard" style={{ animationDelay: "0.1s" }}>
                  <p className="perfCardTitle">{tr("performance.heapTotal", "Heap total")}</p>
                  <p className="perfCardValue">
                    {metrics.memory?.heapTotalMb != null ? `${metrics.memory.heapTotalMb} MB` : "—"}
                  </p>
                  <p className="perfCardSub">{tr("performance.heapTotalSub", "V8 heap total")}</p>
                </div>
                {metrics.memory?.externalMb != null && metrics.memory.externalMb > 0 && (
                  <div className="perfCard" style={{ animationDelay: "0.15s" }}>
                    <p className="perfCardTitle">{tr("performance.external", "External")}</p>
                    <p className="perfCardValue">{metrics.memory.externalMb} MB</p>
                    <p className="perfCardSub">{tr("performance.externalSub", "C++ objects")}</p>
                  </div>
                )}
              </div>
            </section>
            ...
            <section className="perfSection perfSection--animate">
              <h2 className="perfSectionTitle">
                ...
                {tr("performance.apiSection", "API response time (HTTP)")}
              </h2>
              <div className="perfGrid">
                <div className="perfCard" style={{ animationDelay: "0s" }}>
                  <p className="perfCardTitle">{tr("performance.average", "Average")}</p>
                  <p className="perfCardValue">
                  ...
                  <p className="perfCardSub">
                    {metrics.apiResponseTime?.count != null
                      ? tr("performance.lastRequests", "Last {{count}} requests").replace("{{count}}", String(metrics.apiResponseTime.count))
                      : tr("performance.noDataYet", "No data yet")}
                  </p>
                </div>
                <div className="perfCard" style={{ animationDelay: "0.05s" }}>
                  <p className="perfCardTitle">{tr("performance.min", "Min")}</p>
                  <p className="perfCardValue">
                ...
                <div className="perfCard" style={{ animationDelay: "0.1s" }}>
                  <p className="perfCardTitle">{tr("performance.max", "Max")}</p>
                  <p className="perfCardValue">
            ...
            <section className="perfSection perfSection--animate">
              ...
              {metrics.mongo?.error ? (
                ...
                <div className="perfGrid">
                  <div className="perfCard" style={{ animationDelay: "0s" }}>
                    <p className="perfCardTitle">{tr("performance.collections", "Collections")}</p>
                    <p className="perfCardValue">
                      {metrics.mongo?.collections != null ? metrics.mongo.collections : "—"}
                    </p>
                  </div>
                  <div className="perfCard" style={{ animationDelay: "0.05s" }}>
                    <p className="perfCardTitle">{tr("performance.dataSize", "Data size")}</p>
                    <p className="perfCardValue">
                      {metrics.mongo?.dataSizeMb != null ? `${metrics.mongo.dataSizeMb} MB` : "—"}
                    </p>
                  </div>
                  <div className="perfCard" style={{ animationDelay: "0.1s" }}>
                    <p className="perfCardTitle">{tr("performance.storageSize", "Storage size")}</p>
                    <p className="perfCardValue">
                  ...
                  <div className="perfCard" style={{ animationDelay: "0.15s" }}>
                    <p className="perfCardTitle">{tr("performance.indexSize", "Index size")}</p>
                    <p className="perfCardValue">
                      {metrics.mongo?.indexSizeMb != null ? `${metrics.mongo.indexSizeMb} MB` : "—"}
                    </p>
                  </div>
                  <div className="perfCard" style={{ animationDelay: "0.2s" }}>
                    <p className="perfCardTitle">{tr("performance.indexes", "Indexes")}</p>
                    <p className="perfCardValue">
