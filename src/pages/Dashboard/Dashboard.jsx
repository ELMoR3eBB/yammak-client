...
function AnimatedKpiValue({ value, animationKey, loading = false, duration = 760 }) {
  const numeric = Number(value) || 0;
  const display = useAnimatedNumber(loading ? 0 : numeric, animationKey, duration);
  if (loading) return "—";
...
function formatCountdownFromNow(nextAt) {
  if (!nextAt) return "05:00";
  const target = new Date(nextAt).getTime();
  if (Number.isNaN(target)) return "05:00";
...
export default function Dashboard({
  ...
  const [arqamCredit, setArqamCredit] = useState(null);
  const [arqamCountdown, setArqamCountdown] = useState("05:00");
  ...
  const canViewTransactions =
    ...
    perms.includes("cashout.viewAll") ||
    perms.includes("cashout.manage");
  const canViewArqam = perms.includes("*") || perms.includes("arqam.view");
  ...
  const birthdaysTodayResolved = useMemo(() => {
    const today = new Date();
    const m = today.getMonth();
    const d = today.getDate();
    const fromEmployees = (Array.isArray(employees) ? employees : []).filter((emp) => {
      const md = extractMonthDay(emp?.dob);
      return !!md && md.month === m && md.day === d;
    });
    const fromWs = Array.isArray(birthdaysToday) ? birthdaysToday : [];
    if (fromWs.length === 0) return fromEmployees;
    const map = new Map();
    fromWs.forEach((emp) => {
      const key = String(emp?._id ?? emp?.id ?? emp?.workEmail ?? emp?.name ?? Math.random());
      map.set(key, emp);
    });
    fromEmployees.forEach((emp) => {
      const key = String(emp?._id ?? emp?.id ?? emp?.workEmail ?? emp?.name ?? Math.random());
  ...
  const birthdaySpotlightEmployee = useMemo(() => {
    if (!Array.isArray(birthdaysTodayResolved) || birthdaysTodayResolved.length === 0) return null;
    const mine = birthdaysTodayResolved.find((e) => String(e?._id ?? "") === myId);
    return mine || birthdaysTodayResolved[0] || null;
  }, [birthdaysTodayResolved, myId]);
  ...
  useEffect(() => {
    ...
    const maybeFinishInitialListsLoading = () => {
      const employeesReady = !employeesReq.current || employeesReceived.current;
      const rolesReady = !rolesReq.current || rolesReceived.current;
      if (employeesReady && rolesReady) setLoadingLists(false);
    };
    ...
    const unsub = window.api.onWsMessage((msg) => {
      ...
      if (msg?.type === "employees:list" && Array.isArray(msg.employees)) {
        ...
        setLastSyncAt(Date.now());
        maybeFinishInitialListsLoading();
      }
      ...
      if (msg?.type === "arqam:credit:get:result" && msg?.requestId && msg?.ok) {
        setArqamCredit({
          creditBalance: typeof msg.creditBalance === "number" ? msg.creditBalance : null,
          fetchedAt: msg.fetchedAt || null,
          nextRefreshAt: msg.nextRefreshAt || null,
        });
        setArqamCountdown(formatCountdownFromNow(msg.nextRefreshAt));
      }
      if (msg?.type === "arqam:credit:updated") {
        setArqamCredit({
          creditBalance: typeof msg.creditBalance === "number" ? msg.creditBalance : null,
          fetchedAt: msg.fetchedAt || null,
          nextRefreshAt: msg.nextRefreshAt || null,
    ...
    (async () => {
      try {
        ...
        if (canViewArqam) {
          window.api.wsSend({ type: "arqam:credit:get", requestId: rid() });
        }
      } catch {
        setLoadingLists(false);
        setLoadingActivity(false);
  ...
  }, [canSeeEmployees, canSeeRoles, canSeePresence, canManageHoliday, canViewSuggests, canSeeFinance, canViewDrivers, canViewStores, canViewPendingCashout, canViewTransactions, canViewArqam]);

  useEffect(() => {
    if (!canViewArqam) return undefined;
    const timer = setInterval(() => {
      setArqamCountdown(formatCountdownFromNow(arqamCredit?.nextRefreshAt));
    }, 1000);
    return () => clearInterval(timer);
  }, [arqamCredit?.nextRefreshAt, canViewArqam]);

  useEffect(() => {
    onCurrentUserEmployee?.(currentUserEmployee ?? null);
  }, [currentUserEmployee, onCurrentUserEmployee]);

  const loadingRaw = loadingStats || loadingLists;
  const loadingUi = useMinVisibleLoading(loadingRaw);
  const loading = loadingUi;
  const loadingActivityUi = useMinVisibleLoading(loadingActivity);
  const loadingListsUi = useMinVisibleLoading(loadingLists);
  const loadingTopDriversUi = useMinVisibleLoading(loadingTopDrivers);
  const loadingStoreRankingUi = useMinVisibleLoading(loadingStoreRanking);
  const initialBootstrapComplete = !loadingStats && !loadingLists && !loadingActivity;

  useEffect(() => {
    if (!initialBootstrapComplete || initialLoadCompleteRef.current) return;
    initialLoadCompleteRef.current = true;
    onInitialLoadComplete?.();
  }, [initialBootstrapComplete, onInitialLoadComplete]);
      ...
      datasets: [
        {
          ...
          borderColor: isLightTheme ? "rgba(22, 163, 74, 0.88)" : "rgba(34, 197, 94, 0.9)",
          backgroundColor: isLightTheme ? "rgba(22, 163, 74, 0.12)" : "rgba(34, 197, 94, 0.15)",
          fill: true,
          tension: 0.3,
          pointRadius: 4,
          pointHoverRadius: 6,
  ...
  }, [dailyRevenuePoints, isLightTheme, t]);
  ...
  const revenueChartOptions = useMemo(
    () => ({
      ...
      scales: {
        x: {
          grid: { color: isLightTheme ? "rgba(15,23,42,0.12)" : "rgba(255,255,255,0.06)" },
          ticks: { color: isLightTheme ? "rgba(51,65,85,0.9)" : "rgba(255,255,255,0.6)", maxRotation: 45 },
        },
        y: {
          grid: { color: isLightTheme ? "rgba(15,23,42,0.12)" : "rgba(255,255,255,0.06)" },
          ticks: {
            color: isLightTheme ? "rgba(51,65,85,0.9)" : "rgba(255,255,255,0.6)",
    ...
    <div className="dashboardPage">
      ...
      <main className="dashboardMain">
        ...
        <div className="dashboardContent">
          ...
          <section className="dashboardSection dashboardWelcome dashboardSection--from-top">
          ...
          {(canViewStores || canViewDrivers || canViewPendingCashout || canViewTransactions) && (
            <section className="dashboardSection dashboardKpiSummarySection dashboardSection--from-right">
                  ...
                  >
                    ...
                    <div className="dashboardKpiContent">
                      <div className="dashboardKpiValue">
                        <AnimatedKpiValue
                          value={kpiSummary?.storesCount ?? 0}
                          loading={kpiSummary == null}
                          animationKey={`summary-stores-${lastSyncAt ?? 0}`}
                        />
                      </div>
                ...
                {canViewDrivers && (
                  ...
                  >
                    ...
                    <div className="dashboardKpiContent">
                      <div className="dashboardKpiValue">
                        <AnimatedKpiValue
                          value={kpiSummary?.driversCount ?? 0}
                          loading={kpiSummary == null}
                          animationKey={`summary-drivers-${lastSyncAt ?? 0}`}
                        />
                      </div>
                ...
                {canViewPendingCashout && (
                  ...
                  >
                    ...
                    <div className="dashboardKpiContent">
                      <div className="dashboardKpiValue">
                        <AnimatedKpiValue
                          value={kpiSummary?.pendingCashoutCount ?? 0}
                          loading={kpiSummary == null}
                          animationKey={`summary-pending-${lastSyncAt ?? 0}`}
                        />
                      </div>
                ...
                {canViewTransactions && (
                  ...
                  >
                    ...
                    <div className="dashboardKpiContent">
                      <div className="dashboardKpiValue">
                        <AnimatedKpiValue
                          value={kpiSummary?.transactionsToday ?? 0}
                          loading={kpiSummary == null}
                          animationKey={`summary-transactions-${lastSyncAt ?? 0}`}
                        />
                      </div>
          ...
          {canViewArqam && (
            <section className="dashboardSection dashboardArqamSection dashboardSection--from-left">
              <h3 className="dashboardSectionTitle">{t("dashboard.arqamCreditTitle")}</h3>
              <div className="dashboardKpiGrid dashboardKpiGrid--summary">
                <div className="dashboardKpiCard dashboardKpiCard--summary dashboardKpiCard--arqam">
                  <div className="dashboardKpiIcon dashboardKpiIcon--finance">
                    <Wallet size={22} />
                  </div>
                  <div className="dashboardKpiContent">
                    <div className="dashboardKpiValue">
                      {typeof arqamCredit?.creditBalance === "number"
                        ? arqamCredit.creditBalance.toLocaleString()
                        : "—"}
                    </div>
                    <div className="dashboardKpiLabel">{t("dashboard.arqamCreditBalance")}</div>
                    <div className="dashboardArqamCountdown">
                      {typeof arqamCredit?.creditBalance === "number"
                        ? t("dashboard.arqamNextUpdate").replace("{{time}}", arqamCountdown)
                        : t("dashboard.arqamNoData")}
                    </div>
                  </div>
                </div>
              </div>
          ...
          {(canSeeFinance || canViewDrivers) && (
            <SectionWithScrollAnimation
              className="dashboardSectionFinanceMerged dashboardSection--from-left"
              onVisible={() => {
                if (canSeeFinance && window.api?.wsSend) {
                  dailyRevenueReqIdRef.current = rid();
                  window.api.wsSend({
                    ...
                    <div className="dashboardKpiGrid dashboardKpiGrid--financeSingle">
                      <div className="dashboardKpiCard dashboardKpiCard--static">
                        ...
                        <div className="dashboardKpiContent">
                          <div className="dashboardKpiValue">
                            <AnimatedKpiValue
                              value={stats?.companyBalance ?? 0}
                              loading={loading}
                              animationKey={`company-balance-${lastSyncAt ?? 0}`}
                            />
                          </div>
                    ...
                    <div className="dashboardFinanceDriversStripGrid">
                      <div className="dashboardFinanceDriversCard">
                        <span className="dashboardFinanceDriversCardValue">
                          <AnimatedKpiValue
                            value={driverStats?.totalMoneyInHand ?? 0}
                            loading={driverStats == null}
                            animationKey={`drivers-inhand-${lastSyncAt ?? 0}`}
                          />
                        </span>
                      ...
                      <div className="dashboardFinanceDriversCard">
                        <span className="dashboardFinanceDriversCardValue">
                          <AnimatedKpiValue
                            ...
                            loading={driverStats == null}
                            animationKey={`drivers-earnings-${lastSyncAt ?? 0}`}
                          />
                        </span>
                      ...
                      <div className="dashboardFinanceDriversCard">
                        <span className="dashboardFinanceDriversCardValue">
                          <AnimatedKpiValue
                            value={driverStats?.totalWithdrawal ?? 0}
                            loading={driverStats == null}
                            animationKey={`drivers-withdrawal-${lastSyncAt ?? 0}`}
                          />
                        </span>
          ...
          {(canSendHot || canRequestHoliday || canViewReports || canViewDrivers) && (
            <section className="dashboardSection dashboardSection--from-left">
          ...
          {(canManageHoliday || canViewSuggests) && (
            <section className="dashboardSection dashboardSectionPendingApprovals dashboardSection--from-right">
          ...
          <SectionWithScrollAnimation
            className="dashboardSectionTeamSummary dashboardSection--from-top"
            onVisible={() => {
            ...
            <div className="dashboardKpiGrid">
                ...
                >
                  ...
                  <div className="dashboardKpiContent">
                    <div className="dashboardKpiValue">
                      <AnimatedKpiValue
                        value={todaySummary?.reportsToday ?? 0}
                        loading={todaySummary == null}
                        animationKey={`today-reports-${lastSyncAt ?? 0}`}
                      />
                    </div>
                ...
                >
                  ...
                  <div className="dashboardKpiContent">
                    <div className="dashboardKpiValue">
                      <AnimatedKpiValue
                        value={todaySummary?.holidayRequestsToday ?? 0}
                        loading={todaySummary == null}
                        animationKey={`today-holidays-${lastSyncAt ?? 0}`}
                      />
                    </div>
                ...
                >
                  ...
                  <div className="dashboardKpiContent">
                    <div className="dashboardKpiValue">
                      <AnimatedKpiValue
                        value={todaySummary?.suggestionsToday ?? 0}
                        loading={todaySummary == null}
                        animationKey={`today-suggestions-${lastSyncAt ?? 0}`}
                      />
                    </div>
          ...
          <SectionWithScrollAnimation className="dashboardSection--from-right">
                  ...
                  <div className="dashboardKpiContent">
                    <div className="dashboardKpiValue">
                      <AnimatedKpiValue
                        ...
                        loading={loading}
                        animationKey={`overview-employees-${lastSyncAt ?? 0}`}
                      />
                    </div>
                  ...
                  <div className="dashboardKpiContent">
                    <div className="dashboardKpiValue">
                      <AnimatedKpiValue
                        value={totalRoles}
                        loading={loading}
                        animationKey={`overview-roles-${lastSyncAt ?? 0}`}
                      />
                    </div>
              ...
              <div className="dashboardKpiCard dashboardKpiCard--static">
                ...
                <div className="dashboardKpiContent">
                  <div className="dashboardKpiValue">
                    <AnimatedKpiValue
                      value={permCount}
                      animationKey={`overview-perms-${myId || "guest"}`}
                    />
                  </div>
          ...
          <section className="dashboardSection dashboardSectionActivity dashboardSection--from-top">
            <div className="dashboardSectionHead dashboardActivitySectionHead">
              ...
              {!loadingActivityUi && (activityItems.length > 0 || hotOnly.length > 0) && (
                <button
                  type="button"
          ...
          {canSeeEmployees && departmentsCount.length > 0 && (
            <section className="dashboardSection dashboardSection--from-right">
              <div className="dashboardSectionHead">
          ...
          {!loading && canSeeEmployees && canSeeRoles && recentEmployees.length === 0 && recentRoles.length === 0 && (
            <section className="dashboardSection dashboardEmpty dashboardSection--from-top">
          ...
          <div className="dashboardAsideColumn">
            {/* Calendar — dark mode */}
            <div className="dashboardCalendarBlock dashboardBlockEnter dashboardBlockEnter--right">
            ...
            {/* Birthday card: show loading until data ready, then today + upcoming */}
              <div className="dashboardBirthdaysBlock dashboardBirthdaysBlock--card dashboardBlockEnter dashboardBlockEnter--right">
                {loadingListsUi ? (
                ...
                <>
                  {birthdaySpotlightEmployee && (() => {
                    const emp = birthdaySpotlightEmployee;
                          ...
                          <div className="dashboardBirthdayHappyAvatar">
                            <div className="dashboardBirthdayHappyAvatarInner">
                              {hasPhoto ? (
                                ...
                                <span className="dashboardBirthdayHappyInitial">{initial}</span>
                              )}
                            </div>
                          </div>
                        ...
                        <div className="dashboardBirthdayUpcomingAvatar">
                          {(() => {
                            ...
                            return hasPhoto ? (
                              ...
                              <span className="dashboardBirthdayUpcomingInitial">{initial}</span>
                            );
                          })()}
                        </div>
            ...
            <div className="dashboardPresenceBlock dashboardBlockEnter dashboardBlockEnter--right">
            ...
            {canViewStores && (
              <div className="dashboardTopDriversBlock">
                <div className="dashboardTopDriversBlockCard">
                  ...
                  <div className="dashboardTopDriversCard">
                    {loadingStoreRankingUi && storeRanking.length === 0 ? (
                      <ul className="dashboardTopDriversList" aria-busy="true" aria-label={t("common.loading")}>
