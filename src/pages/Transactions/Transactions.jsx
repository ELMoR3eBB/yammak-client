...
function typeLabel(row) {
  if (row?._transactionKind === "cashin") {
    if (row.type === "driver") return "Driver";
    if (row.type === "store") return "Store";
    return "Other";
  }
...
export default function Transactions({ account }) {
  ...
  useEffect(() => {
    ...
    const unsub = window.api.onWsMessage((msg) => {
      ...
      if (msg?.type === "wallet:adjust:result" && msg?.ok) {
        fetchList();
      }
    });
    (async () => {
  ...
  const normalizedCashins = useMemo(() => {
    return cashins.map((c, idx) => ({
      ...
      type: c.type,
      userName: c.type === "driver" ? (c.driverName || "") : c.type === "store" ? (c.storeName || c.source || "") : (c.source || ""),
    ...
    <div className="auditLogsPage cashoutListPage transactionsPage">
      ...
      {detailRow && (
        <TransactionDetailModal
          detailRow={detailRow}
          detailBackdropRef={detailBackdropRef}
          detailExiting={detailExiting}
